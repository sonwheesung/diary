...
export type AiFail =
  ...
  | 'out-of-range'
  /**
   * 그 기간의 글이 `MAX_INPUT_CHARS`(4만 자)를 넘었다. **모델 앞에서 끝나 캡을 쓰지 않는다.**
   *
   * 🔴 이 줄이 **빠져 있었다**(2026-08-25). 서버는 처음부터 이걸 돌려주는데 union에 없어서
   *   `t('report.fail.too-large')`가 키를 못 찾고 화면에 **`report.fail.too-large`가 날것으로**
   *   떴다. `check:i18n`은 못 잡는다 — 호출이 `t(\`report.fail.${reason}\`)`라 동적이다.
   *
   * ⚠ 실무에서 닿기 어렵다(주간 기준 **하루 5,100자를 이레 내내** 써야 한다). 그래도
   *   닿으면 **되돌릴 길이 없다** — 잘라 보내는 것도, 나눠 만드는 것도 없다(§10.3).
   */
  | 'too-large'
 ...
 *   서버에는 남아 있었고(당시 90일, 2026-09-15 부터 탈퇴 시까지) **가져올 길만 없었다.**
 *
...
export async function fetchStoredReport(
  ...
  if (!res.ok) {
    // 404(`not-found`)가 정상 경로다 — 탈퇴·삭제 요청으로 지워졌거나 애초에 저장에 닿기 전에 끊겼다
    return null;
  }
  let json: Record<string, unknown>;
  try {
 ...
 *   글은 서버에 탈퇴 시까지 남아 있다(2026-09-15). 이 경로가 그것을 되살린다.
 *

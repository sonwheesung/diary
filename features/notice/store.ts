...
import { activeServer } from '@/lib/common-server/client';
...
export const useNoticeStore = create<NoticeState>((set, get) => ({
  ...
  load: async () => {
    ...
    /*
     * 어느 인스턴스로 부를지는 `activeServer()` 가 정한다 — **포그라운드 하트비트와 같은 규칙**이라야
     * 한 사람이 하루에 subject 두 행을 찍지 않는다(`docs/SUPPORT_SYSTEM.md` §3.1).
     *

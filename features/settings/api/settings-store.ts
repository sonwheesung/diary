...
export const SETTING_KEYS = {
  ...
  proUntil: 'pro_until',
  /**
   * 🔴 **마지막으로 알던 구독 만료 시각**(ISO). `pro_until`과 **다른 칸**이다.
   *
   * 구독이 끝나면 `pro_until`은 지워진다 — 지난 시각을 캐시로 두면 다음 실행에서
   * 만료 판정이 흔들린다. 그런데 **백업 파기 예정일(만료 + 90일)은 그 시각이 있어야
   * 셀 수 있다.** 그래서 끄면서 여기에 따로 적는다.
   *
   * ⚠ 이 값으로 권한을 판정하지 않는다. 오직 **유예 배너·삭제 예정일**만 읽는다.
   * ⚠ 로그아웃·탈퇴 때 `pro_until`과 **함께** 비운다 — 다음 사람에게 앞사람의
   *   파기 예정일이 보이면 안 된다.
   */
  proExpiredAt: 'pro_expired_at',
  /**

 ...
 *   **낡았다**: 2026-08-13 결정(§5.2)으로 `ai_reports`에 본문을 ~~90일~~ 탈퇴 시까지(2026-09-15) 보관한다.
 ...
 * ⚠ ~~90일이 지나면 리퍼가 지운다~~ 2026-09-15 부터 리퍼는 리포트를 안 지운다. 탈퇴나 삭제 요청 뒤에는 `not-found` 이고 그게 정상이다.
 */
export async function GET(req: Request): Promise<Response> {
  ...
  try {
    /*
     * ⚠ `(subject_id, kind, period_key)` 인덱스가 없다. 이 테이블은 사용자·기간마다 한 줄 남짓이라 작고(2026-09-15 부터 탈퇴 시까지 쌓인다)
  ...
  try {
    ...
    try {
      await db.insert(aiReports).values({
         ...
         *   로컬 리포트에서 낸다(~~서버는 90일치만 갖고 있어 애초에 못 낸다~~ 서버가 낼 이유가 없다).
         */
        /*

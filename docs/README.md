...
| **연령 게이트** | ✅ | 2026-08-27 — 처리방침 §2 가 *"만 14세 미만 아동은 로그인 기능을 이용할 수 없습니다"* 를 **현재형으로** 15개 언어·공개 웹에 게시했는데 **코드가 0줄**이었다(고지가 코드를 앞질렀다, CLAUDE.md §12 2026-08-24 와 같은 종류). 🔴 **부팅이 아니라 `signIn()` 안**에 세운다 — 조각은 로그인이 선택적이라(기둥 1) 부팅 게이트는 일기만 쓰는 사람에게 나이를 묻는다. 공용 §3.1 이 요구하는 것은 순서가 아니라 **경계**(미달자의 소셜 식별자를 받기 전)다. 지역별 **16/14/13 · 판정 불가 16** · 보수 판정식 · **생년 미저장** · 서버 전송 0 ([`AUTH_SYSTEM.md`](./AUTH_SYSTEM.md) §1) |

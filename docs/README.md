...
| 🔴 **운영 앱 구글 로그인이 안 된다** | 2026-09-04 실기기 — vc18에서 `(10) DEVELOPER_ERROR`. 기기 APK 서명 `48:3A:B1:E9…`가 Cloud Console에 **등록돼 있지 않다**(등록된 것은 `CD:86:BB:DB…` 하나뿐이고 8/19 앱 이전 뒤에 만든 클라이언트가 없다). **테스터 43명 전원**이 문의·구독·백업·AI를 못 쓴다. → `com.son0925.jogak` + `48:3A:B1:E9…` 안드로이드 클라이언트를 만들면 **재빌드 없이** 열린다(전파 5분~수 시간). 상세는 [`SUPPORT_SYSTEM.md`](./SUPPORT_SYSTEM.md) |

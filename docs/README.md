...
| | |
|---|---|
| 스토어 등록정보 | 🔴 **`ko-KR` 하나였다.** 한국어 개선 + **`en-US` 신설**(제목·간단·자세한 설명). 문안 정본은 `assets/store/build-listing.mjs` — **12개 언어를 다 써뒀고** 길이 검사를 통과한다(콘솔 입력은 순차 진행) |
...
⏭ 남은 것: ① 나머지 11개 언어 등록정보 입력 ② 영어 스크린샷 → 기본 언어 전환.

...
| **운영 콘솔** `/ops-7c1d94` | ✅ | 2026-08-13 — 대시보드·AI 사용량·**리포트 품질**·백업 금고·정리 5탭. **읽기 전용**. 🔴 *개인 비특정*은 2026-09-04에 **"입력으로는 쓰고 출력으로는 안 낸다"** 로 다시 그었다 — *"리포트가 별로예요"* 문의를 열 수 없으면 CS가 성립하지 않고, 처리방침이 이미 *"90일 전 삭제를 문의로 요청"* 까지 약속해뒀다. 훑어보기(subject 목록)는 여전히 안 만든다. `npm run check:admin` 30개. 🔴 **상수 단일화 2개**(2026-09-04) — 콘솔이 `aiEffort` 기본값을 따로 `low`로 적어놔 **실제로는 medium인데 화면이 low라고 말했다**(§5 *"정책은 화면이 아니라 코드 상수다"*). [`ADMIN_SYSTEM.md`](./ADMIN_SYSTEM.md) |
...
# 운영 콘솔을 건드렸으면
npm run check:admin            # 30개 — **fail-closed** · 헤더 판정 · KST 집계 창 · 원가 추정
...
| 🔴 **경로 간** — 두 코드 경로가 같은 사실에 같은 답을 내는가 | **0개** |
| **문서 대조** — 문서에 박힌 수와 실제 수 | `check:doc-counts` |

🔴 세 번째 칸이 **0개**다. 앞의 둘로는 원리적으로 못 잡는다 — 공통서버가 정리해 준 한 문장이 이유를 다 설명한다:

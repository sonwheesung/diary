...
| **AI 리포트 — 그 기간의 모양** | ✅ | 2026-08-25 — 리포트 상세에 **요일 격자**(주간) · **주/달별 막대**(월간·연간) · 감정 범례. 🔴 **모델을 안 부르고 저장도 안 한다** — 화면이 그릴 때 로컬에서 세므로 **옛 리포트에도 붙는다**(모델이 만드는 지표는 캡이 평생 1번이라 소급 불가). 차트 라이브러리 0. 에뮬레이터에서 둘을 고쳤다: 막대를 감정색으로 칠했더니 `angry` 6일인 주가 **빨간 벽**이 됐고(→ 단색), 월간 축 `2026년 32주`가 다섯 칸에 안 들어갔다(→ 시작일 숫자) ([`AI_REPORT_SYSTEM.md`](./AI_REPORT_SYSTEM.md) §8.3) |

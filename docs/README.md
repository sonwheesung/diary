...
| **본문 텍스트 서식** | ✅ | 2026-08-24 — 문단마다 **정렬·크기(H1~H4/본문)·굵기·글자색**. 🔴 **색은 hex가 아니라 코드**(`rose`)로 저장하고 `theme/palettes.ts`의 `ink`가 테마별 실제 값을 준다 — hex로 저장하면 라이트에서 고른 색이 **다크에서 안 읽히고** 스킨을 갈아끼워도 옛 조각만 옛 색으로 남는다. 🚫 **글꼴 종류는 뺐다**(한글 폰트 굵기당 ~1.5MB · 나머지 14개 언어에서 조용히 폴백). `npm run check:diary-format` 38개. **에뮬레이터 실동작 확인**(2026-08-24, ⏭ 실기기는 v10부터 — dev 빌드가 스토어 서명과 충돌한다): 가운데 문단만 분할·저장 후 재열람·다크모드 색 반전·베트남어/독일어 시트. 🔴 그 과정에서 **서식보다 오래된 커서 버그**를 잡았다 — 프로그램이 블록을 쪼갤 때 RN이 쏘는 `onSelectionChange`가 커서를 앞 블록으로 끌어가 **사진을 연달아 넣으면 두 번째가 엉뚱한 자리**에 들어갔다([`DIARY_SYSTEM.md`](./DIARY_SYSTEM.md) §1.1) |
...
| 항목 | 내용 |
|---|---|
| 🔴 **연령 게이트가 없는데 처리방침이 있다고 말한다** | `legal-text.ts:168` 처리방침 §2 가 *"※ 만 14세 미만 아동은 로그인 기능을 이용할 수 없습니다"* 를 **현재형으로** 적고 **15개 언어·공개 웹**에 나가 있는데, `signIn()` 은 곧장 `GoogleSignin.signIn()` 을 부르고 **연령 문자열이 `locales/*.json` 에 0개**다 — 확인도 차단도 없다. CLAUDE.md §12(2026-08-24) *"고지가 코드를 앞질렀다"* 와 같은 종류. ⚠ **한국(14)만 보고 만들면 안 된다** — EEA 를 뺀 145개국이라 미국(13)이 들어온다. 지역별 기준·중립 입력·보수 판정은 [`common/GLOBAL_DATA_COMPLIANCE.md`](file:///C:/project/common/GLOBAL_DATA_COMPLIANCE.md) §2·§3, 이식 사례는 §11(배구명가). 조각 행은 §12 |
...
| 🔴 **조각 서버 DB가 정지돼 있다 — 백업·AI가 죽어 있다** | 2026-09-02 발견. 운영 프로필이 `https://jogak-stg.vercel.app` 를 가리키는데 그 DB(`jogak-stg`, 무료 조직)가 **8/28부터 정지**다 — Delvewarden 에 슬롯을 내주려고 수동으로 멈췄고(그때는 옳은 판단), 그 뒤 **vc16 이 백업·AI 를 담은 채 나가면서 전제가 깨졌다.** 테스터 43명에게 백업·복원·AI 리포트가 전부 실패하는 상태이고 **원격 관측이 0이라 5일간 아무도 몰랐다.** 일기·잠금·광고는 로컬이라 무사하다. ⏭ 임시: `volleyball-stg` 와 스왑(배구는 운영 DB 가 Pro 조직에 따로 있다). ⏸ **2026-09-03 사용자 결정 — 당장은 복구하지 않는다**(개발 검증이 서버를 안 지난다). 🔴 **그래도 프로덕션 차단은 그대로다** — 돈 내고 산 사람에게 구독 세 혜택 중 둘이 첫날부터 죽는다. 🔴 본해법: 운영 프로젝트를 Pro 조직에 신설(+$10/월) — `CLAUDE.md` §5 · [`common/BUSINESS_INFO.md`](file:///C:/project/common/BUSINESS_INFO.md) §5.1 |
...
npm run check:ai               # 108개 — ISO 주차·주차↔날짜범위 왕복·프롬프트·인젝션 방어·스키마·동의
                               #   + 백필 지평·하위 완비(15개)
...
# 본문 서식·블록을 건드렸으면
npm run check:diary-format     # 38개 — 저장 형태(기본값 미저장)·**서식이 다르면 병합 금지**·

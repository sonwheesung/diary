...
| 출시 뒤 남은 것 | |
|---|---|
| 🔴 **첫 문의가 60시간 미답변** | 2026-09-04 21:49 접수 · `bug` · android · **`app_version 0.2.5`**. 서버 `tickets_stale` 알림이 울고 있다(공통서버 세션 9/7 실측). ⚠ **그날 실기기 검증에서 우리가 보낸 것일 가능성이 높다** — 같은 vc18(=0.2.5)에서 로그인 → **문의 전송** → 내 문의 조회를 확인했다. 내용을 열어 보기 전에는 단정하지 않는다 |

...
### 🔴 숫자를 "조각 전체 활성 사용자"로 읽지 않는다

세션은 **로그인으로만** 생기는데, 조각은 로그인이 **선택**이다(`CLAUDE.md` §4). 일기 작성·잠금·
캘린더·검색은 로그인 없이 돌아가고, 로그인이 필요한 것은 문의·구독·백업·AI 리포트뿐이다.

→ 여기서 잡히는 DAU는 **"로그인까지 한 사용자 중 오늘 앱을 켠 수"** 다. 비로그인 사용자는
한 명도 안 잡힌다. 서버에 잡힌 조각 subject가 5명이라는 것도 실사용자 수가 아니라 **로그인한
사람 수**다. **버그가 아니라 구조다** — 대시보드를 보는 사람이 이걸 모르면 출시 판단을 그르친다.

⚠ 한 가지 더 좁다: `notice/store.ts`의 `load()`는 `loaded` 플래그로 **프로세스당 1회**만 돈다.
안드로이드가 프로세스를 며칠씩 살려두면 **콜드 스타트 없이 쓰는 날은 안 세어진다.** 포그라운드
복귀에 하트비트를 붙이면 닫히지만 그건 호출부 변경이라 이번 범위 밖이다.

⏭ 비로그인까지 세려면 `registerDevice`로 기기 subject를 발급해야 하는데, 그러면 **같은 사람이
로그인 전후로 subject 2개**가 되어 중복 계상되고 문의 이력도 갈린다. 서버에 subject 병합 개념이
없어 **서버 설계 변경이 선행**한다 — 별건이다.

...
⏭ **아직 정하지 않은 것**: stg 패키지명(`com.son0925.jogak.stg` 형태) · AdMob을 stg에서
아예 끌지(테스트 단위만 쓰므로 실제 앱 ID가 필요 없을 수 있다) · RC 프로젝트 분리 여부.
**패키지명은 정하면 되돌리기 비싸다** — Play에 한 번 올라간 패키지명은 재사용 불가다.

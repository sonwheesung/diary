...
~~⚠ 단 §4.4의 어긋남은 남아 있다~~ → ✅ **해소(2026-08-24)** — 탈퇴가 AI 테이블을 지운다.
...
③ [완료 2026-08-24] §4.4 탈퇴 ↔ AI 테이블 정합       POST /api/v1/ai/purge
...
### 🔴 실제로 눌러보고 드러난 것 — §7의 2단계 항목이 틀렸다 (2026-08-24)

이 문서는 2단계를 *"사용자가 데이터 삭제를 요청할 수 있는 방법을 제공하나요? → **예**"* 로 적었다.
**콘솔의 실제 질문은 그것이 아니다.**

```
사용자가 계정 및 관련 데이터의 삭제를 요청하는 데 사용할 수 있는 링크 추가
   [https://sonwheesung.github.io/diary/delete-account.html]   ← 별도 필드. 이미 채워져 있었다
                                                                 = §4의 "메커니즘"은 여기서 충족된다

사용자가 계정을 삭제하지 않고도 데이터의 일부 또는 전체를 삭제하도록
요청할 수 있는 방법을 제공하나요?  (선택사항)
   ○ 예   ● 아니요   ○ 아니요(하지만 90일 이내 자동 삭제됨)
```

두 번째가 **선택사항**이고 훨씬 좁다 — *"계정을 삭제하지 않고도"* 다.

**`예`를 고르면 전용 URL 필드가 필수로 열리고**, 그 링크는 세 조건을 충족해야 한다:
① 개발자/앱 이름 기재 ② **부분 삭제 요청 절차를 눈에 띄게 표시** ③ 삭제·보관 데이터 유형과 기간.

`delete-account.html`은 **계정 삭제** 안내라 ②를 적고 있지 않다. 없는 페이지를 억지로 링크하지
않고 **`아니요`로 두었다.** 계정 삭제 배지(`앱 계정 삭제`)는 위 URL로 이미 확보돼 있어 손실이 없다.

⚠ **판단을 한 번 뒤집었다가 되돌렸다.** 처리방침 §9(*"삭제 요구 … 연락처로 서면 또는 전자우편 …
지체 없이 조치"*)가 Play가 예시로 든 *"contact forms, or a dedicated email alias"* 와 맞아
`예`로 바꿨는데, **콘솔이 요구하는 것은 창구가 아니라 그 절차를 적은 링크**였다.

⏭ 부분 삭제 요청 절차를 담은 페이지를 만들면 그때 `예`로 바꾼다. 처리방침 §9가 이미 근거를
   갖고 있어 문안은 대부분 있다 — 필요한 것은 **그것만 따로 보여주는 페이지**다.

### 실제 제출 기록 (2026-08-24)

| | |
|---|---|
| 2단계 | 손대지 않음(위 이유). 암호화 `예`·OAuth·계정 삭제 URL은 이미 옳았다 |
| 3단계 | **사용자 ID · 구매 내역 · 기타 사용자 제작 콘텐츠** 3개 체크. `사진`·`건강 정보`는 안 함 |
| 4단계 | 셋 다 **수집만 · 일시처리 아니요 · 사용자 선택 가능**. 목적은 §7 그대로 |
| 5단계 | 미리보기 확인 — 공유 목록에 **기타 UGC 없음**, 수집 목록에 새 3개, `사진` 없음 |
| 저장 | ✅ 게시 개요에 **`검토를 위해 변경사항 1개 제출`** 로 잡혔다(리로드 확인) |
| 검토 전송 | ✅ ~~보류 — v10 릴리스와 함께 보낸다~~ → **기다릴 필요가 없었다.** 저장하자 **혼자 제출·출시됐다**(제출 3 · 8/24 21:57 제출 → 22:07 출시됨 · 소스 `API`). 🔴 **데이터 보안은 심사 대기열을 타지 않는다** — 관리형 게시가 꺼져 있으면 폼 저장이 곧 게시다. *"검토 중 업로드는 큐 뒤로 밀린다"* 는 **AAB 얘기**인데 그것을 폼에까지 적용해 하루를 미뤘다. 그 하루가 그대로 **선언과 배포본이 어긋난 기간**이다(§0) |

⚠ 기존 6개 카드는 열지도 않았다. 이메일 주소 카드가 실수로 열렸을 때는 **값을 확인만 하고
   변경 없이 닫았다**(수집만·비일시·선택·앱 기능+계정 관리 — §3.2 #1과 일치).

// 공통 서버 클라이언트 SDK.
// ⚠️ 원본: common_server/client/index.ts 에서 복사 (2026-09-02, 웜 스타트 하트비트본).
//    이 파일은 손으로 고치지 말 것 — 서버 계약이 바뀌면 원본을 갱신하고 다시 복사한다.
//    (앱 4~5개 규모에선 monorepo·npm 패키지 오버헤드가 이득보다 크다는 판단 — common_server 규약)
//
//    2026-09-02 재복사 사유: `heartbeat()` 신설. 하트비트가 `fetchBootstrap()` 에만 얹혀
//    있었는데 그건 **JS 프로세스당 1회**라, 홈 버튼이 프로세스를 안 죽이는 안드로이드에서
//    **웜 스타트가 통째로 빠졌다** — 자주 쓰는 사용자일수록 덜 잡히는 거꾸로 된 오차였다.
//    🔴 조각은 인스턴스가 둘이라(로그인 칸·기기 칸) **리스너를 한 곳에만** 붙이고
//      `signedIn ? commonServer : deviceServer` 로 고른다 — 기기 세션 고정은 로그인한 사람을
//      매일 2중 계상한다(`docs/SUPPORT_SYSTEM.md` §3.1). 토큰은 `exp` 를 들고 다니고
//      옛 토큰은 `iat` 폴백이라 기존 로그인이 안 끊긴다.
//
//    2026-09-01.2 재복사 사유: 세션 토큰에 **갱신 경로가 없었다** — `iat + 180일` 고정
//    카운트다운이라 매일 써도 그날이 오면 죽고, bootstrap 은 무효 토큰을 401 없이 조용히
//    무시하므로 그 사용자는 **DAU 에서 영구히 사라지고 앱도 서버도 모른다.** 이제 bootstrap
//    응답에 `session.token` 이 실릴 수 있고(발급 30일 경과 시) SDK 가 조용히 교체한다.
//
//    2026-09-01 재복사 사유: `fetchBootstrap()` 이 세션을 실어 보낸다(활성 지표).
//    ⚠ 조각은 로그인이 **선택**이라 이것만으로는 로그인한 사용자만 잡힌다 —
//      비로그인 집계는 기기 subject 가 필요하다(`docs/SUPPORT_SYSTEM.md` §3.1).
//
